"""Hex Encoder/Decoder - Encode and decode hexadecimal strings."""
...
def create_ui(frame, COLORS, FONTS):
    def encode():
        ...
        formatted = " ".join(hex_str[i:i+2] for i in range(0, len(hex_str), 2)).upper()
        show_output(formatted)

    def decode():
        text = input_box.get("1.0", "end-1c").strip().replace(" ", "")
        try:
            bytes.fromhex(text)
        except ValueError:
            show_output("Error: Invalid hex string")
            return
    ...
    ctk.CTkLabel(frame, text="Hex Encoder / Decoder", font=FONTS["title"], text_color=COLORS["text"]).pack(anchor="w", padx=10, pady=(10, 5))
